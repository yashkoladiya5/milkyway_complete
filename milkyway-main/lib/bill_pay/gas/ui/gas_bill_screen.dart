...
class _GasBillScreenState extends State<GasBillScreen> {
  ...
  Future<void> _selectBillSource() async {
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () {
                  ...
                  _pickAndScanBill(ImageSource.camera);
                },
              ),
              ListTile(
                ...
                onTap: () {
                  Navigator.pop(context);
                  _pickAndScanBill(ImageSource.gallery);
                },
              ),
            ],
          ),
  ...
  Future<void> _pickAndScanBill(ImageSource source) async {
    ...
    try {
      ...
      if (response['success'] == true && response['bill'] != null) {
        ...
        final controller = Provider.of<GasBillScreenController>(context, listen: false);

        setState(() {
          controller.gasProviderController.text = bill['provider'] ?? 'Gujarat Gas Limited';
          
          if (controller.selectedCategory == 1) {
            controller.mobileController.text = bill['customerNo'] ?? '';
          } else {
            controller.customerIdController.text = bill['customerNo'] ?? '';
          }
        });

        // Trigger verify
        if (controller.selectedCategory == 1) {
          await controller.checkMobile(context);
        } else {
          await controller.checkCustomerId(context);
        }

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Gas Details Auto-Populated by AI! 🧠'))
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('AI Scan failed to extract details. Enter manually.'))
        );
      }
    } catch (e) {
      try {
  ...
  Future<void> makePayment(String price) async {
    try {
      ...
      print("=== GAS PAYMENT DEBUG ===");
      print("selectedCategory: ${gasController.selectedCategory}");
      print("payCustomerData: ${gasController.payCustomerData}");
      print("bookingOperatorData: ${gasController.bookingOperatorData}");
      print(
          "customerIdController.text: ${gasController.customerIdController.text}");
      print(
          "gasProviderController.text: ${gasController.gasProviderController.text}");
      print("mobileController.text: ${gasController.mobileController.text}");

      if (gasController.selectedCategory == 1) {
        // Gas Cylinder Booking - update payment status in BOTH tables
        print("Updating Gas Booking (category 1)");
        if (gasController.bookingOperatorData.isNotEmpty) {
          String mobile =
              gasController.bookingOperatorData['registeredMobile'] ??
                  gasController.mobileController.text;
          String provider =
              gasController.bookingOperatorData['gasProviderName'] ??
                  gasController.gasProviderController.text;
          String? customerId = gasController.bookingOperatorData['customerId'];

          // Update GasBooking table (paymentStatus)
          await dbHelper.updateGasBooking(
            mobile: mobile,
            provider: provider,
            status: "Paid",
          );

          // ALSO update payGasBill table (set billAmountRemain to 0)
          if (customerId != null) {
            print(
                "Also updating payGasBill table with customerId: $customerId");
            await dbHelper.updatePayGasBill(
        ...
        // Pay Gas Bill - update bill amount to 0
        print("Updating Pay Gas Bill (category 0)");
        if (gasController.payCustomerData.isNotEmpty) {
          print("Calling updatePayGasBill with:");
          print(
              "  customerId: ${gasController.payCustomerData['customerId'] ?? gasController.customerIdController.text}");
          print(
              "  provider: ${gasController.payCustomerData['gasProviderName'] ?? gasController.gasProviderController.text}");
          await dbHelper.updatePayGasBill(
            customerId: gasController.payCustomerData['customerId'] ??
                gasController.customerIdController.text,
            provider: gasController.payCustomerData['gasProviderName'] ??
                gasController.gasProviderController.text,
            amount: 0.0,
          );
        } else {
          print("ERROR: payCustomerData is empty!");
        }
      }
      // Record as expense in wallet
      await dbHelper.insertWalletData(
        model: CartWalletModel(
          id: DateTime.now().millisecondsSinceEpoch,
          name: gasController.selectedCategory == 1
              ? "Gas Cylinder Booking - ${gasController.gasProviderController.text}"
              : "Gas Bill Payment - ${gasController.gasProviderController.text}",
          price: "₹$price",
        ...
        builder: (context, value, child) {
          return InkWell(
            onTap: () async {
              double? amount;
              if (value.selectedCategory == 1) {
                amount = value.bookingOperatorData["billAmountRemain"] != null
                    ? double.tryParse(
                        value.bookingOperatorData["billAmountRemain"].toString())
                    : null;
              } else {
                amount = value.payCustomerData["billAmountRemain"] != null
                    ? double.tryParse(
                        value.payCustomerData["billAmountRemain"].toString())
                    : null;
              }
              ...
              if (amount != null && amount > 0) {
                ...
                Provider.of<GasBillScreenController>(context, listen: false)
                    .clearData();
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("Your Bill is Already Paid ✅")));
                Navigator.pop(context);
              }
            },
            child: Container(
