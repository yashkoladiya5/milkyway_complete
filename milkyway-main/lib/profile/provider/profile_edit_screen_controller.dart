    ...
    if (status.isGranted) {
      try {
        ...
        if (photo != null) {
          ...
          final response = await BackendApi.uploadFile(
            userId: userId,
            file: _selectedFile!,
          );

          if (response['success'] == true) {
            _imageUrl = response['imageUrl']?.toString() ?? '';
            Future.microtask(() {
              Navigator.pop(context);
            });
          } else {
            print('Error uploading file: ${response['message']}');
          }
    ...
    if (Platform.isIOS) {
      ...
      if (status.isGranted) {
        try {
          ...
          print("Error capturing image from gallery : $e");
        }
      } else if (status.isDenied) {
        print("Camera permission denied");
      } else if (status.isPermanentlyDenied) {
