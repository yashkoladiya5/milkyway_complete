    ...
    if (userId != null && userId.isNotEmpty) {
      try {
        await BackendApi.putJson(
          ...
          body: {
            'autoPayId': prefs.getString(SharedPreferenceKeys.autoPayId) ?? '',
            'autoPayBalanceId':
                prefs.getString(SharedPreferenceKeys.autoPayBalanceId) ?? '',
            'dailyProductIds':
                prefs.getStringList(SharedPreferenceKeys.dailyProductIdKey) ?? [],
            'locationId': prefs.getString(SharedPreferenceKeys.locationIdKey) ?? '',
            'selectedPlan': prefs.getString(SharedPreferenceKeys.planKey) ?? '',
            'userData': prefs.getString(SharedPreferenceKeys.userDataKey) ?? '',
          },
        );
      } catch (_) {
        // Logout should still proceed even if preference sync fails.
      }
    }
