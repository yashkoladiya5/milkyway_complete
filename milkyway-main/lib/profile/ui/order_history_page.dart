...
class _OrderHistoryPageState extends State<OrderHistoryPage> {
              ...
              builder: (context, value, child) {
                return SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemCount:
                        value.ordersOnlyList.isEmpty ? 1 : value.ordersOnlyList.length,
                    itemBuilder: (context, index) {
                      if (value.ordersOnlyList.isEmpty) {
                        return Center(
                            child: Padding(
                      ...
                      String? image = value.ordersOnlyList[index].image;
                      String? name = value.ordersOnlyList[index].name;
                      String? weight = value.ordersOnlyList[index].weightValue +
                          " " +
                          value.ordersOnlyList[index].weightUnit;
                      String? price = value.ordersOnlyList[index].price;
                      return Padding(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              _buildTransactionImage(image, name ?? ""),
                              Column(
  ...
  Widget _getPlaceholderIcon(String name) {
    String lowerName = name.toLowerCase();
    if (lowerName.contains("recharge") || lowerName.contains("mobile")) {
      return const Icon(Icons.phone_android, size: 40, color: Colors.white);
    } else if (lowerName.contains("electricity") || lowerName.contains("bill")) {
      return const Icon(Icons.electric_bolt, size: 40, color: Colors.white);
    } else if (lowerName.contains("water")) {
      return const Icon(Icons.water_drop, size: 40, color: Colors.white);
    } else if (lowerName.contains("gas")) {
      return const Icon(Icons.local_fire_department, size: 40, color: Colors.white);
    } else if (lowerName.contains("dth") || lowerName.contains("cable")) {
      return const Icon(Icons.tv, size: 40, color: Colors.white);
    } else {
      return const Icon(Icons.shopping_bag, size: 40, color: Colors.white);
    }
  }

  Widget _buildTransactionImage(String? imageUrl, String name) {
    bool isInvalidUrl = imageUrl == null ||
        imageUrl.isEmpty ||
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: isInvalidUrl
            ? LinearGradient(
                begin: Alignment.topLeft,
                end: Alignment.bottomRight,
                colors: [
                  HexColor(AppColorsLight.orangeColor),
                  HexColor(AppColorsLight.redColor).withOpacity(0.8),
                ],
              )
            : null,
        color: isInvalidUrl ? null : Colors.grey[200],
      ),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(15),
        child: isInvalidUrl
            ? Center(child: _getPlaceholderIcon(name))
            : Image.network(
                imageUrl!,
                fit: BoxFit.cover,
                errorBuilder: (context, error, stackTrace) {
                  return Container(
                    ...
                    child: Center(child: _getPlaceholderIcon(name)),
                  );
                },
