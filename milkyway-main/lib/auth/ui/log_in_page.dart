...
import 'package:milkyway/services/auth_legacy_bridge.dart';
...
class _LogInPageState extends State<LogInPage> {
  ...
  logInUser() async {
    ...
    try {
      final response = await AuthLegacyBridge.loginWithFallback(
        mobileOrEmail: _contactController.text,
      ...
      if (response['success'] == true) {
        ...
        if (preferences is Map<String, dynamic>) {
          prefs.setString(
              SharedPreferenceKeys.autoPayBalanceId,
              preferences['autoPayBalanceId']?.toString() ?? '');
          prefs.setString(SharedPreferenceKeys.autoPayId,
              preferences['autoPayId']?.toString() ?? '');
          final dailyProductIds = preferences['dailyProductIds'];
          if (dailyProductIds is List) {
            prefs.setStringList(
              SharedPreferenceKeys.dailyProductIdKey,
              dailyProductIds.map((e) => e.toString()).toList(),
            );
          }
          prefs.setString(SharedPreferenceKeys.locationIdKey,
              preferences['locationId']?.toString() ?? '');
          prefs.setString(SharedPreferenceKeys.planKey,
              preferences['selectedPlan']?.toString() ?? '');
        }
        ...
        Navigator.pushReplacement(
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(response['message']?.toString() ?? "Please Enter Valid Number Or Password")));
          userCheck = 0;
      }
    } catch (e) {
