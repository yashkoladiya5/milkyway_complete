...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Widget _buildPaymentTextField({required TextInputType keyboardtype}) {
    return Container(
      ...
      height: height * 0.045,
      // Removed hardcoded width to allow flexibility
      // width: width * 0.900,
      decoration: BoxDecoration(
  ...
  Future<void> makePayment(String price) async {
    try {
      // Parse the price as double and round it to get a whole number in INR
      int roundedPrice = double.parse(price).round();
      print("ROUNDED PRICE FOR PAYMENT: $roundedPrice");
      paymentIntent = await createPaymentIntent(roundedPrice, 'INR');
  ...
  Future<Map<String, dynamic>> createPaymentIntent(
    ...
    try {
      Map<String, dynamic> body = {
        'amount': (amount * 100)
            .toString(), // Stripe requires amount in cents (integer)
        'currency': currency,
